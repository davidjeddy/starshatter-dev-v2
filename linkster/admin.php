...
######### CHECK COOKIE #########
if ($admin_password == $valid_password)
	{
		$valid = 1;
	}
else
...
if (!$valid)
	{
		...
		echo "<input type=password name=login_password size=12 class=\"input\">\n";
		echo "<input type=hidden name=submit_login value=1>\n";
		echo "<input type=submit value=' Log In ' class=\"input\">\n";
		echo "</p>\n";
	}
...
if ($valid)
	{
		...
		echo "    <li><a href=$PHP_SELF?action=add_cat class=\"minilinks\">Add a Category</a></li>\n";
		echo "    <li><a href=$PHP_SELF?action=modify_cat class=\"minilinks\">Modify a Category</a></li>\n";
		echo "    <li><a href=$PHP_SELF?action=remove_cat class=\"minilinks\">Remove a Category</a></li>\n";
		echo "    </td>\n";
		echo "    <td>\n";
		echo "    <div id=linkster-title-small>Links Admin</div>\n";
		echo "    <li><a href=$PHP_SELF?action=add_link class=\"minilinks\">Add a Link</a></li>\n";
		echo "    <li><a href=$PHP_SELF?action=modify_link class=\"minilinks\">Modify a Link</a></li>\n";
		echo "    <li><a href=$PHP_SELF?action=remove_link class=\"minilinks\">Remove a Link</a></li>\n";
		echo "    </td>\n";
		echo "  </tr>\n";
		echo "</table>\n";
	}
				...
				if (!$error)
					{
						...
						mysql_query("INSERT INTO $cat_table (Category, Description) VALUES ('$Category','$Description')");  // executes query
					}
				else
					{
		...
		if (!$CID)
			{
				...
				echo " <input type=submit value=' Select ' class=\"input\">\n";
				echo "</p>\n";	
				echo "</form>\n";
			}
						...
						if (!$error)
							{
								$Category = escape_quotes($Category);
								$Description = escape_quotes($Description);
								mysql_query("UPDATE $cat_table SET Category = '$Category', Description = '$Description' WHERE ID = $CID");  // executes query
							}
						else
							{
								$Category = strip_quotes($Category);
...
if ($action == "remove_cat" && $valid) {
  ...
  if ($CID) {
    if (!$remove && !$cancel) {
      $query = mysql_query("SELECT Category, $cat_table.Description FROM $cat_table, $link_table WHERE $cat_table.ID = $link_table.Cat_ID AND $cat_table.ID = $CID");
      $link_count = mysql_num_rows($query);
      if ($link_count == 0) {  // run new query if no links exist
        $query = mysql_query("SELECT Category, Description FROM $cat_table WHERE ID = $CID");
      }
      ...
      echo "<input type=submit name=remove value=' Remove ' class=\"input\"> <input type=submit name=cancel value=' Cancel ' class=\"input\">\n";
      echo "</form>\n";
    }
     else {  // process form
      if ($remove) {
...
if ($action == "add_link" && $valid) {
  ...
  if (!$submit || $error) {
    echo "<form method=post action=$PHP_SELF?$QUERY_STRING>\n";
    link_form($db, $Cat_ID, $CID, $Website, $URL, $Description, $cat_table, $link_table);
...
if ($action == "modify_link" && $valid) {
  ...
  if (!$CID & !$LID) {
    ...
    category_select($db, '', '', $cat_table);  // print categories
    echo "</p>\n";
    echo "<p><div id=linkster_title_small>Sort Order:</div><span style='font-size: 8pt'><input type=radio name=order value=alpha checked class=\"input\">Alphabetically<br>\n<input type=radio name=order value=chrono class=\"input\">Chronologically</span></p>\n";
    echo "<input type=submit value=' Select ' class=\"input\">\n";
    echo "</form>\n";
  }
  ...
  if ($CID && !$LID) {
    ...
    $query = mysql_query("SELECT $link_table.ID, Website, URL, Category FROM $cat_table, $link_table WHERE $cat_table.ID = $link_table.Cat_ID AND Cat_ID = $CID ORDER BY $orderby");
    print_title_message("Modify a Link", "Select a link to modify.");
    echo "<p>\n";
    while (list($LID, $Website, $URL) = mysql_fetch_array($query)) {
      echo "<div><b>[ <a href=$PHP_SELF?action=$action&LID=$LID class=\"minilinks\">Modify</a> ]</b> <a href=$URL class=\"minilinks\">$Website</a><div>\n";  
    }
    echo "</p>\n";
  }
  ...
  if ($LID) {
    ...
    if ($submit) {
      ...
      if (!$error) {
        ...
        $URL = escape_quotes($URL);
        $Description = escape_quotes($Description);
        mysql_query("UPDATE $link_table SET Cat_ID = '$CID', Clicks = '$Clicks', Website = '$Website', URL = '$URL', Description = '$Description' WHERE ID = $LID");
      }
      else {
        $Website = strip_quotes($Website);
        $URL = strip_quotes($URL);
    ...
    if (!$submit || $error) {
      ...
      echo "<form method=post action=$PHP_SELF?$QUERY_STRING>\n";
      link_form($db, $Cat_ID, $CID, $Website, $URL, $Description, $cat_table, $link_table);
      echo "<p><div id=linkster-title-small>Clicks:</div><input type=text name=Clicks size=4 maxlength=6 value=\"$Clicks\" class=\"input\"></p>\n";
      echo "<input type=submit name=submit value=' Modify Link ' class=\"input\">\n";
      echo "</form>\n";
    }
  }
}
...
if ($action == "remove_link" && $valid) {
  ...
  if ($CID && !$LID) {
    ...
    echo "<p>\n";
    while (list($LID, $Website, $URL) = mysql_fetch_array($query)) {
      echo "<div><b>[ <a href=$PHP_SELF?action=$action&LID=$LID class=\"minilinks\">Remove</a> ]</b> <a href=$URL class=\"minilinks\">$Website</a><div>\n";  
    }
    echo "</p>\n";
  }
  if ($LID && !$remove && !$cancel) {
    ...
    $query = mysql_query("SELECT * FROM $link_table WHERE ID = $LID");
    extract(mysql_fetch_array($query));
    echo "<ul><a href=$URL class=\"minilinks\">$Website</a> ($Clicks clicks)<br>\n$Description</ul>\n";
    echo "<form method=post action=$PHP_SELF?$QUERY_STRING>\n";
    echo "<input type=hidden name=CID value=\"$Cat_ID\" class=\"input\">\n";
    echo "<input type=hidden name=Website value=\"$Website\" class=\"input\">\n";
    echo "<input type=submit name=remove value=' Remove ' class=\"input\"><input type=submit name=cancel value=' Cancel ' class=\"input\">\n";
    echo "</form>\n";
  }
  else {  // process form
