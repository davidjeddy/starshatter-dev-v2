...
//	************************************************************
	...
	function FastTemplate ($pathToTemplates = "")
	{
		...
		$this->FILELIST		=	array();	//	Holds the array of filehandles
										//	FILELIST[HANDLE] == "fileName"

		$this->DYNAMIC		=	array();	//	Holds the array of dynamic
										//	blocks, and the fileHandles they
										//	live in.

		$this->PARSEVARS	=	array();	//	Holds the array of Variable
										//	handles.
										//	PARSEVARS[HANDLE] == "value"

		$this->LOADED		=	array();	//	We only want to load a template
										//	once - when it's used.
										//	LOADED[FILEHANDLE] == 1 if loaded
										//	undefined if not loaded yet.

		$this->HANDLE		=	array();	//	Holds the handle names assigned
										//	by a call to parse()

		$this->ROOT			=	"";			//	Holds path-to-templates

		$this->WIN32		=	false;		//	Set to true if this is a WIN32 server

		$this->ERROR		=	"";			//	Holds the last error message

		$this->LAST			=	"";			//	Holds the HANDLE to the last
										//	template parsed by parse()

		$this->STRICT		=	true;		//	Strict template checking.
